...
class GistClient {
  ...
  _axios: { request: (config: any) => Promise<any> }
  ...
  async get<T = any>(url: string, config: GistConfig = {}): Promise<T> {
    return this.request({ ...config, method: 'get', url })
  }

  async delete<T = any>(url: string, config: GistConfig = {}): Promise<T> {
    return this.request({ ...config, method: 'delete', url })
  }

  async post<T = any>(url: string, data?: any, config: GistConfig = {}): Promise<T> {
    return this.request({ ...config, method: 'post', url, data })
  }

  async put<T = any>(url: string, data?: any, config: GistConfig = {}): Promise<T> {
    return this.request({ ...config, method: 'put', url, data })
  }

  async patch<T = any>(url: string, data?: any, config: GistConfig = {}): Promise<T> {
